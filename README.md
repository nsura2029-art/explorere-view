...
- **Ice-tap sound (taps only):** an original ~1 s "ice tap" (glassy tink, tiny ice pings, a
  shimmering bell highlight, frosty fade — 5 variations, a random one per tap; generated by
  `scripts/generate_ice_tap.py`). It plays when a finger, pen or mouse press is **released without
